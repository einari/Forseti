...
{
    public class when_applying_a_file_with_global_dependencies : given.a_configuration_file_reader
    {
        ...
        const string harness_dependency_file_path = "Dependencies/first_dependency";
        const string global_dependency_file_path = "Global/firdependency";
        ...
        It should_add_a_harness_with_harness_dependency_file_path = () => result.Dependencies.ShouldContain(file => file.FullPath == harness_dependency_file_path);
        It should_add_a_harness_with_global_dependency_file_path = () => result.Dependencies.ShouldContain(file => file.FullPath == global_dependency_file_path);
        It should_add_a_harness_with_dependencies_in_the_correct_order = () =>
                                                                             {
                                                                                 result.Dependencies.First().ShouldEqual((File)global_dependency_file_path);
                                                                                 result.Dependencies.Last().ShouldEqual((File)harness_dependency_file_path);
                                                                             };
    }
}